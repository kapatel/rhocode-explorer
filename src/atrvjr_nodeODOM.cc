...
//    cout << " orientation.z: " << msg.orientation.z <<endl;
    ...
    cin >> userDist;
    cout << "Please enter a turning angle";
    cin >> userAngle;

    //While we have moved less distance(plus GPS error) than the goal distance
//    while(msg.position.x <= odomX)
    while(userDist > (odomX+2))
    {
      //go right if within pi - 3pi/2
//      if((msg.orientation.z >= PI) && (msg.orientation.z < ((7*PI)/4)))
//        if(msg.orientation.z < -PI/4)
	if(userAngle > PI && userAngle < ((7*PI)/4))
	{
           msgOut.linear.x = 0.2;
           msgOut.angular.z = -0.15;
	}
      //go left if within pi/4 - pi
//      else if(/*msg.orientation.z < PI && */msg.orientation.z > (PI/4))
	else if(userAngle < PI && userAngle > PI/4 )
	 {
            msgOut.linear.x = 0.2;
            msgOut.angular.z = 0.15;
	 }
...
//When we exit the while loop stop the robot
      msgOut.linear.x = 0;
      msgOut.angular.z = 0;
        chatter_pub.publish(msgOut);
