...
void ATRVJR::getSonarPoints(const int ringi, sensor_msgs::PointCloud* cloud) const {
    ...
    for (int i = 0; i < numSonar; ++i) {
        if (readings[i] < SONAR_MAX_RANGE/ (float) RANGE_CONVERSION) {
         ...
         //   angle *= M_PI / 180.0;			//degrees to radians conversion
	      double angle = SONAR_RING_ANGLES[i];
	      if (angle < 0)
		{
		angle += 2*M_PI;			//convert negative radians to 2pi
	      ...
	      x /= 1000;
	      y /= 1000;
	      double sonar_ring_distance[26];
	      sonar_ring_distance[i] = sqrt((x*x)+(y*y));
            //  sonar_ring_distance[i] = sqrt((SONAR_RING_X_DISTANCE[i])+(SONAR_RING_Y_DISTANCE[i]));
	      double d = sonar_ring_distance[i] + readings [i];
