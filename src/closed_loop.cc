...
/**
 * Modified on 10/21/2013 by Keval Patel
 *
 * Closed Loop Controlled for ATRV JR
 * By Noam Zilberstein 08/2012
 * 
 * This program subscribes to the robot's odometry messages and a heading command which
 * tells it where to go relative to its current position and North.  It then uses a PD
 * control loop to publish linear and angular velocity commands to the robot.
 */
...
//The robot's location taken from odometry
float odomX, odomY, odomA;
//Distance and heading to current destination
float heading_distance, heading_angle; 
bool updateHeading = true;  //tells the controller is the current destination should be corrected
...
int main(int argc, char** argv)
{
   ...
   * ----------------------- */
    ...
    while (ros::ok())
    {
    	...
    	{
    		// Publish message to halt the rover
    		d = 0;
    		msgOut.linear.x = 0;
    		msgOut.angular.z = 0;
