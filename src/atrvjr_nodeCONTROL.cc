...
int main(int argc, char** argv) {
   ...
   * ----------------------- */
  float k = 3.5;     // Spring Constant (N/m)
  float B = 5;     // Damping Constant
...
//    cout << " orientation.z: " << msg.orientation.z <<endl;
    ...
    float pathSlope = getSlope(0, xDest, 0, yDest);
    float xPath = getIntersectX(pathSlope, pathSlope+PI/2, 0, x, 0, y);
    xPath -= ds * sin(pathSlope);
    float yPath = getY(xPath, pathSlope, 0, 0);
    dp = sqrt((xPath-x)*(xPath-x) + (yPath-y)*(yPath-y));
    ...
    while (d > 1 && ros::ok()) {
	 // Process a round of subscription messages
      ...
      xPath = getIntersectX(pathSlope, pathSlope+PI/2, 0, x, 0, y);
      if (xPath > xDest)
      ...
      if (vTrans < -.5) vTrans=-.5;
      //Brake Function so that the robot slows down before reaching destination
      if (d < 3.2) vTrans *= .95;
